...
(function(global) {
     ...
     * @ignore
     */
    _argv.checkArg = function(arg, dataTypeArray, optional) {
        var i, l=dataTypeArray.length, 
            group, instance, 
            dataType, size, 
            returnValue = false;
        ...
        for(i=0; i<l; i++) {
            instance = dataTypeArray[i];
            // check the validity of datatype instance
            if (!simpli.isFunction(instance)) {
            ...
            if (_argv.isType(arg, dataType, size)) {
                returnValue = true;
            }
        }
    };
     ...
     * @ignore
     */
    _argv.setCallbackArg = function(callbackArg, argI, 
     ...
     * @param {integer} signLen     The length of argument signature array
     * @param {boolean} repeatable  Whether there is repeatable arguments
     * @param {function} callback   The callback fucntion
     * @param {object} callbackObj  The argument object to be provided to the 
     *                              callback function
     * @throws {TypeError}
     * @ignore
     */
    _argv.invalidHandler = function(invalidArg, 
        ...
        // determine the invalid type
        if (invalidArg.type == simpli.argv.UNEXPECTED_DATATYPE) {
            invalidMessage = "Expected '"+invalidArg.expected+"', '"+
                invalidArg.given+"' given";
            index = invalidArg.index;
        } else if (invalidArg.type == simpli.argv.MISMATCH_ARGNUMBER) {
        ...
        if (!simpli.isNull(stackTrace)) {
            ...
            for (i=0,l=stackTrace.length; i<l; i++) {
                ...
                if (stackTrace[i].invokedBy === invokedBy || 
                    ...
                    // update the callback argument object
        ...
        // update the message of callback argument
     ...
     * @ignore
     */
    _argv.MismatchArgNumber = function() {
        return {
            type: simpli.argv.MISMATCH_ARGNUMBER, 
            index: -1, 
            expected: "", 
            given: ""
        };
    };
    _argv.UnexpectedDataType = function(i, dataTypeArray, arg) {
        return {
            type: simpli.argv.UNEXPECTED_DATATYPE, 
            index: i, 
     ...
     * @param {function} callback   The callback function for determining the 
     *                              arugment validity
     * @param {interger} [size]     (Optional) The upperbound size of the 
     *                              array (if the datatype is an array)
     * @memberof simpli
     */
    simpli.defineType = function(callback, size) {
        if (simpli.isDefined(size) && 
            (size==='[]' || simpli.isInteger(size))) {

            // custom array data type
            return (function(arg) {
                // check array size
                if (size !== '[]' && arg.length > size) { return false; }
                // check for validity of array elements
                for(var i=0; i<arg.length; i++) {
                    if (!callback(arg[i])) { return false; }
                }
            });
        } else {
            return callback;
    ...
    simpli.argv = function(args) {
        ...
        // prepare argument signature array
        if (thisArgsLen > 2 || 
            (thisArgsLen === 2 && simpli.isString(thisArgs[1]))) {

            // list of data type signature in list of arguments
            signatures = thisArgs;
            signatures.shift();
        } else {
            // list of data type signature in array
        ...
        // prepare the template for each argument in callback argument
        for(i=0; i<argsLen; i++) {
            // push an argument template into callback argument
        ...
        for(signI=0,argI=0; signI<signLen; signI++) {
            ...
            if (_argv.optionalRegEx.test(signInstance)) {
                optional = true;
                // remove the "[]"
                ...
                if (signI > argsLen-1) {
                    ...
                    if (simpli.isNull(invalidArg)) {
                        invalidArg = _argv.MismatchArgNumber();
                    }
                }
            }
            ...
            if (signInstance.slice(0,3) === "...") {
                ...
                repeatUntil = argsLen-(signLen-signI-1);
                repeatable = true;
                // remove the "..."
            ...
            if (argI < repeatUntil) {
                ...
                while(argI < repeatUntil) {
                    if (!_argv.checkArg(args[argI], dataTypeArray, optional)) {
                        // incorrect data type
                        invalidObj = _argv.UnexpectedDataType(
                        ...
                        _argv.setCallbackArg(callbackArg, argI, false, args[argI], 
                ...
                if (_argv.checkArg(args[argI], dataTypeArray, optional)) {
                    ...
                    if (!optional || 
                        ...
                        // update the callback argument to include the status
                        // of currebt arguments
                        _argv.setCallbackArg(callbackArg, 
                            argI, true, args[argI]);
                    } 
                } else {
                    // incorrect data type
        ...
        if (argI < argLen) {
            // arguments provided are more than the signatures specified
             ...
             */
            invalidObj = _argv.MismatchArgNumber();
            // only record the first invalid argument
            if (simpli.isNull(invalidArg)) {
                invalidArg = _argv.MismatchArgNumber();
            }
            // update all the ramaining argument to invalid
            for (i=argI; i<argLen; i++) {
                _argv.setCallbackArg(callbackArg, i, false, args[i], 
                    '', '', invalidObj.type);
            }
        }
        ...
        if (simpli.isNull(invalidArg)) {
            ...
            _argv.invalidHandler(invalidArg, 
                invokedBy, args, signLen, repeatable, 
                callback, callbackArg);
            return false;
        }
    ...
    /**
     * @property {string} simpli.argv.MODE_STRICT    Unexpected Datatype
     * @memberof simpli.argv
     */
    simpli.argv.UNEXPECTED_DATATYPE = "{UNEXPECTED_DATATYPE}";
    /**
     * @property {string} simpli.argv.MODE_STRICT    Mismatch argument number
     * @memberof simpli.argv
     */
    simpli.argv.MISMATCH_ARGNUMBER = "{MISMATCH_ARGNUMBER}";
    /**
     * @property {string} simpli.argv.MODE_STRICT    STRICT mode
     * @memberof simpli.argv
     */
    simpli.argv.MODE_STRICT = "{MODE_STRICT}";
    /**
     * @property {string} simpli.argv.MODE_EXTEND    EXTEND mode
     * @memberof simpli.argv
     */
